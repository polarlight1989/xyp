...
class SumAmountjxssc
{
	...
	public function winTest($periodsNumber,$num,$debug=false){ 
		#$periodsNumber = 10799104; 
		#$result = $this->Formula();  
		...
		for ($i=0; $i<count($result); $i++)
		{
			...
			$ts = $result[$i]['g_distribution_1'] + $result[$i]['g_distribution_2'] + $result[$i]['g_distribution_3'];
			if($ts>0) $getgwin  = $getgwin*(1-$ts*0.01); 
			$realMoney += $getgwin;
			#$this->db->query("UPDATE `g_zhudan` SET `g_win` = '{$getgwin}' {$mx} WHERE `g_id` = {$result[$i]['g_id']} LIMIT 1 ", 2);
